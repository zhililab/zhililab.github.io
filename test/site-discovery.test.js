...
test('source preserves www as the canonical origin', () => {
  assert.match(read('_config.yml'), /^url: https:\/\/www\.zhililab\.cn$/m);
  assert.equal(read('source/CNAME').trim(), 'www.zhililab.cn');
  assert.match(
    read('source/privacy/index.md'),
    /\[www\.zhililab\.cn\]\(https:\/\/www\.zhililab\.cn\/\)/
  );
});
...
test('source publishes crawler discovery files', () => {
  ...
  assert.match(
    robots,
    /^Sitemap: https:\/\/www\.zhililab\.cn\/sitemap\.xml$/m
  );
